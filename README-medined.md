...
The `deploy-text-response-application.sh` script will deploy a simple web application so that you can see that cert-manager and your other K8S components are working.

Before running the script, read it to review the configuration that is needed. You'll need to set `K8S_DOMAIN_NAME`, `TEXT_RESPONDER_SUBDOMAIN_NAME`, and `ACME_EMAIL` environment variable. Also, you'll need to verify that `Z26RNL4JYFTOTI` is the correct hosted zone id.
